...
export const Explore = ()=>{
    ...
    return(
        <div>
            <h1>{category}</h1>
            <input placeholder="Search" onChange={(e)=> dispatch({type:"searchVideos", payload: e.target.value})}/>
            <div>
                {
                    state.data?.map(video =><div>
                        {<div>
                        <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTY9vC4avPe99O1R8r2P2wEwGC1CnxuufqoKlf9L82Iqghv18Qk4KJBCcbuBc5AfSqhHak&usqp=CAU" height="30px" alt="save to watch later" onClick={()=> dispatch({type : "updateWatchLater", payload:video})}/>
                            <NavLink to={video.src}><img src={video.thumbnail} height="300px" alt={video.title}/>
                            <h3>{video.title}</h3>
                            <h3>{video.category}</h3>
                            <p>{`${video.views} views | ${video.creator}`}</p></NavLink>
                            </div>
                        }
                    </div>)
                }
